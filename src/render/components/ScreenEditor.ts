...
export const CREATE_RECT = 1
export const MOVING_RECT = 2
...
export class ScreenEditor extends Event {
    ...
    drawAnchors(w: number, h: number, margin: number, scaleFactor: number, radius: number) {
        if (this.disabled) {
            return
        }
    ...
    onMouseMove(e: any) {
        ...
        if (this.selectRect) {
            ...
            if (this.anchors) {
                this.anchors.forEach(([x, y]: any, i: number) => {
